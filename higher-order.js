...
//Code Here
let evenNumbers  = mixedNumbers.filter((el, val, arr) => el % 2 === 0)
...
//Code Here
let postTaxPrices  = prices.map((el, index, arr) => el + el * 0.07);
...
let totalPopulation  = populations.reduce((total, val) => total = total + val)
...
//Code Here
let myStrongest = monstersInYourPocket.filter((val, index, arr) => val.CP > 200)
...
let ordersTotal = orders.map((val) => {
  ...
  return sum += val.price + val.price * val.tax;
});
...
let bobsTotal = purchases.map((val, i, arr) => {
  let total = 0;
